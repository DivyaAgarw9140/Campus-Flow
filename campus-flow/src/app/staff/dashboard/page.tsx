...
export default function StaffDashboard() {
  ...
  const delayedOrders = orders.filter(o => 
    o.status === 'PREPARING' && 
    (new Date().getTime() - new Date(o.created_at).getTime()) / 60000 > 15
  ).length;
...
<div className="text-right">
    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">AI Target</p>
    <p className={`text-sm font-black ${
      (new Date().getTime()-new Date(order.created_at).getTime())/60000 >15
      ? 'tezt-red-500 animate-pulse'
      : 'text-green-400'
    }`}>
      Must Ready in : 16ms
    </p>
